...
class EntryReader:
    ...
    def dec(self):
        blowfish = Blowfish()
        while self.read_offset < EntryReader.DECODED_SIZE:
            byteval = self.read(8)
    ...
    def enc(self) -> bytes:
        ...
        while read_offset < EntryReader.DECODED_SIZE:
    ...
    def build_save_bytes(self, byte_array: bytes) -> bytes:
        assert len(byte_array) == EntryReader.DECODED_SIZE
        crc = self.build_crc(byte_array)
        head = b"\x00\xfc\x07\x00"
        result = bytearray(head)
        result += byte_array[: EntryReader.HALF_SIZE]
        result += crc
        result += byte_array[EntryReader.HALF_SIZE :]
        return bytes(result)


class HeadEntryReader:
    FILE_SIZE = 432
    ...
    def to_int_byte_field(self, offset: int, length: int):
        return IntByteField(
            length,
            int.from_bytes(self.data_buffer[offset : offset + length], "little"),
            offset,
        )
    ...
    def build_save_bytes(self) -> bytes:
        ...
        result = bytearray(head)
        result += self.data_buffer[: HeadEntryReader.HALF_SIZE]
        result += crc
        result += self.data_buffer[HeadEntryReader.HALF_SIZE :]
        return bytes(result)
